...
export function ContactStrip(): JSX.Element {
  return (
    <div className="hidden sm:flex items-center gap-1 mr-1" aria-label="Pritika's contact links">
      {LINKS.map((l) => (
        <a
          key={l.label}
          href={l.href}
          ...
          aria-label={l.label}
          title={l.label}
          className="inline-flex items-center justify-center h-9 w-9 rounded-full text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/[0.04] transition-colors"
        >
          {l.icon}
        </a>
      ))}
